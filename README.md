...
- 🔄 **Multi-format conversion**: Symbol, Ensembl ID, and Entrez ID interconversion (NEW in v3.2.0)
...
- 🔀 **Ortholog mapping**: Cross-species gene conversion
- 🧫 **Seurat integration**: Direct gene sync for Seurat objects
...
### Gene Conversion (v3.2.0)

```r
library(geneSync)

# Convert gene symbols to standardized symbols (default)
...
# NEW: Convert from Ensembl ID to symbol
gene_convert(c("ENSG00000111640", "ENSG00000141510"), species = "homo", from = "ensembl_id")
#>               input symbol symbol_authority gene_id      ensembl_id chromosome    match_type final_symbol
#> 1 ENSG00000111640  GAPDH            GAPDH    2597 ENSG00000111640         12 ensembl_match        GAPDH
#> 2 ENSG00000141510   TP53             TP53    7157 ENSG00000141510         17 ensembl_match         TP53

# NEW: Convert from Entrez ID to symbol
gene_convert(c("2597", "7157"), species = "homo", from = "gene_id")
#>   input symbol symbol_authority gene_id      ensembl_id chromosome match_type final_symbol
#> 1  2597  GAPDH            GAPDH    2597 ENSG00000111640         12   id_match        GAPDH
#> 2  7157   TP53             TP53    7157 ENSG00000141510         17   id_match         TP53

# NEW: Convert symbols to Ensembl IDs
...
### Cross-Species Ortholog Conversion
...
# NEW (v3.2.0): Convert from Ensembl IDs (common in 10x data)
obj <- gene_sync_add_to_obj(obj, species = "homo", from = "ensembl_id", to = "symbol")

# NEW (v3.2.0): Convert from Entrez IDs
...
|----------|-------------|
| `gene_convert()` | Convert gene symbols ↔ IDs ↔ Ensembl IDs |
...
| chromosome | Chromosome location |
| match_type | Type of match (exact_authority, exact_symbol, synonym, ensembl_match, not_found, etc.) |
...
| `ensembl_match` | Matched by Ensembl ID (NEW in v3.2.0) |
| `not_found` | No match found |
| `invalid_id` | Invalid numeric ID |
| `invalid_ensembl` | Invalid Ensembl ID format (NEW in v3.2.0) |

## Changelog

### v3.2.0 (2026-02-25)
- **NEW**: `from` and `to` parameters in `gene_convert()` for flexible format conversion
- **NEW**: Support for Ensembl ID as input format
- **NEW**: Support for Entrez ID as input format
- **NEW**: `.ensembl_to_info_fast()` internal function for Ensembl ID lookup
- **NEW**: `from` parameter in `gene_sync_add_to_obj()` for Seurat objects with Ensembl IDs
- **DEPRECATED**: `direction` parameter (use `from`/`to` instead, backward compatible)

### v3.0.0 (2026-02-24)
- Added `update_features` parameter
- Added `keep` column (yes/no/others)
- Added `duplication` column
- Improved duplicate gene handling with detection rate
